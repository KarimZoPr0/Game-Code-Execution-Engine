  ...
  return (
    <>
      <div className="h-14 bg-[#161b22] border-b border-[#30363d] flex items-center justify-between px-3 shrink-0">
        ...
        {/* Center - Logo + Project */}
        <div className="flex items-center gap-2">
          <img src="/favicon.png" alt="CodeForge" className="w-7 h-7 rounded-md" />
          <span className="font-semibold text-sm text-[#c9d1d9] max-w-[120px] truncate">
            {currentProject?.name || 'CodeForge'}
          </span>
        </div>

        {/* Right section - Build + Profile */}
        <div className="flex items-center gap-2">
          <button
            onClick={handleBuildAndRun}
            disabled={isBuilding}
            className="p-2 rounded-lg bg-[#238636] hover:bg-[#2ea043] text-white disabled:opacity-50"
          >
            {getBuildIcon()}
          </button>
          <ProfileMenu />
      ...
      <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
        <SheetContent side="left" className="w-[280px] p-0 bg-[#161b22] border-[#30363d]">
          <SheetHeader className="px-4 py-4 border-b border-[#30363d]">
            <SheetTitle className="flex items-center gap-2 text-[#c9d1d9]">
